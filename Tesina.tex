...
\newpage	
	...
	\end{figure}
 	...
 	$\frac{K}{1+T_1 s} e^{-\tau s}$, dove $e^{-\tau s}$ è un fattore di ritardo.\\\\Per far ciò si utilizzerà un tool di stima della risposta a gradino presente nel toolbox di System Identification di Matlab. Il risultato è riportato nella pagina successiva.\\
 	\includegraphics[scale=0.33,angle=90]{P2_estimating}\\\\
	...
	Attraverso l' uso della funzione "tuning\textunderscore migliore.m", (appendice) è stato determinato il metodo più efficace, attraverso l' analisi della risposta della funzione ad anello chiuso $W=\frac{\text{PID}P2}{1+\text{PID}P2}$,di cui si riporta il risultato nella pagina seguente.\\
