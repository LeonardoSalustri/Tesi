...
\usepackage{float}
\hypersetup{pdftitle={Modellazione di un generatore sincrono e controllo della tensione in una rete elettrica multi-macchina},pdfauthor={Leonardo Salustri}}

\title{Modellazione di un generatore sincrono e controllo della tensione in una rete elettrica multi-macchina}
...
\copyyear{2019}
\advisor{Prof. Alessandro Di Giorgio}
\tutor{Prof. Alessandro Di Giorgio}
\examdate{15 Ottobre 2019}
...
\begin{document}
	...
	Un grazie a tutte le persone che prendono parte tutti i giorni a questo mio lungo percorso,\\
	un grazie a Francesco per il suo continuo contraddirmi, che mi ha spinto a imparare sempre qualcosa in più,\\
	un grazie a mia nonna e mia zia per la loro innata disponibilità e per avermi reso la vita più semplice,\\
	e infine un ringraziamento va ai miei genitori, su tutti, per avermi permesso di arrivare fin qui.
	\end{acknowledgments}
	\begin{abstract}
	L' obiettivo di questo lavoro è effettuare uno studio della macchina sincrona attraverso un approccio graduale. Il lettore difatti sarà guidato attraverso tre capitoli che si focalizzano su aspetti diversi. Prima di tutto verranno derivate le equazioni differenziali per costruire un modello adatto a descrivere il generatore sincrono in un contesto isolato a partire dal suo principio di funzionamento. Questo modello verrà inserito in un contesto multi-macchina, e si andrà a lavorare su un modello con lo spazio di stato unico che tiene conto della dinamica dei generatori e delle interconnessioni della rete. Il modello verrà linearizzato intorno a un punto di lavoro, e nel terzo capitolo ci si concenterà sul controllo di tensione delle macchine in un' ottica d' analisi per piccoli segnali. Il fine è quindi quello di dimostrare l' efficacia dei controllori standard, quale il PID, nel controllo del sistema d' eccitazione del generatore. Si suppone che la rete stia lavorando in condizioni di equilibrio, e ci si chiede come debba essere sintetizzato l' apparato di controllo al fine di reagire a delle perturbazioni minime delle variabili. Cosa si intende per "perturbazioni minime"? L' idea di base è quella di considerare come minime quelle perturbazioni che fanno sì che la dinamica della macchina possa essere rappresentata da funzioni linearizzate intorno al punto di lavoro, senza commettere errori di entità rilevante. La stima di quanto effettivamente debba essere l' entità delle perturbazioni, nel caso di sistemi complessi, è solitamente lasciata all' esperienza del controllista.\vfill
\end{abstract}
	Di seguito il glossario contenente la descrizione dei simboli usati.\\\\
	\resizebox{0.59\textheight}{!}{
	\begin{tabular}{|l|l|l|}
		\hline
		\textbf{Simbolo} & \textbf{Descrizione}\\
		\hline
		$\Psi_{i}\text{ i=}a,b,c$ & Flusso del campo magnetico attraverso la fase i\\
		$\Psi_{f}$ & Flusso del campo magnetico attraverso il circuito di field\\
		$\Psi_{Q}$ & Flusso del campo magnetico attraverso il circuito di damping\\
		$\Psi_{i}\text{ i=}o,q,d$ & Flusso del campo magnetico attraverso il circuito fittizio i\\
		$V_i\text{ i=}a,b,c$ & Tensione della fase i\\
		$V_f$ & Tensione del circuito di field\\
		$V_i\text{ i=}o,d,q$ & Tensione del circuito fittizio i\\
		$V_n$ & Tensione del neutro \\
		$V_B$ & Valore della tensione di base di statore per la normalizzazione\\
		$V_{FB}$ & Valore della tensione di base del circuito di field\\
		$V_{QB}$ & Valore della tensione di base del circuito di damping\\
		$I_i\text{ i=a,b,c}$ & Corrente circolante nella fase i\\
		$I_f$ & Corrente circolante nel circuito di field\\
		$I_Q$ & Corrente circolante nel circuito di damping\\
		$I_i\text{ i=o,d,q}$ & Corrente circolante nel circuito fittizio i\\
		$I_B$ & Valore della corrente di base di statore per la normalizzazione\\
		$I_{FB}$ & Valore della corrente di base del circuito di field\\
		$I_{QB}$ & Valore della corrente di base del circuito di damping\\
		$L_{ii}\text{ i=a,b,c}$ & Autoinduttanza della fase i\\
		$L_{ij}\text{ i=a,b,c j=a,b,c}$ & Mutue induttanze tra le fasi ij\\
		$L_n$ & Induttanza del neutro\\
		$l_i\text{ i=d,q,f,Q}$ & Induttanza di perdita del circuito i\\
		$M_s$ & Valore delle mutue induttanze tra le fasi\\
		$M_f$ & Valore massimo delle mutue induttanze tra fasi e field\\
		$M_Q$ & Valore massimo delle mutue induttanze tra fasi e damping\\
		$P$ & Matrice di trasformazione di Park\\
		$r_i\text{ i=}a,b,c$ & Resistenza della fase i\\
		$R_{abc}$ & Matrice delle resistenze di statore\\
		$r_f$ & Resistenza circuito di field\\
		$r_Q$ & Resistenza circuito di damping\\
		$r_n$ & Resistenza circuito di neutro\\
		$\omega_i$ & Velocità angolare del rotore della macchina i-esima\\
		$\omega_B$ & Velocità angolare del rotore (377 $\frac{\text{rad}}{s}$)\\
		$C_m$ & Coppia meccanica esercitata dalla turbina sul rotore\\
		$C_e$ & Coppia elettrica accelerante esercitata dal carico sul rotore\\
		$D$ & Coefficiente di attrito\\
		$\delta_i$ & Sfasamento della macchina i-esima dal riferimento\\
		$\delta_{ij}$ & Differenza tra $\delta_i$ e $\delta_j$\\
		$\theta$ & Rotor angle della macchina\\
		$W_r$ & Energia della macchina che ruota a velocità costante $\omega_B$\\
		$x_i\text{ i=d,q}$ & Reattanza sincrona in diretta o quadratura\\
		$x'_i\text{ i=d,q}$ & Reattanza transitoria in diretta o quadratura\\
		$\tau'_{i0}\text{ i=d,q}$ & Costante di tempo a circuito aperto in d,q\\
		$\Psi'_i\text{ i=d,q}$ & Flussi in d,q durante il transitorio\\
		$E'_{i}\text{ i=d,q}$ & Tensioni in d,q durante il transitorio\\
		$E'_t$ & Tensione della macchina dietro reattanza transitoria\\
		\hline
	\end{tabular}}
	\tableofcontents
	...
	\mainmatter
	\chapter{La macchina in un contesto isolato}
	...
	La matrice di trasformazione che ci consente di cambiare il nostro riferimento è [1]:\\
	...
	4. Le mutue induttanze tra i circuiti rotorici sono nulle ($L_{fQ}=L_{Qf}=0$) poiché i circuiti sono ortogonali fra loro;\\
	...
	\begin{figure}
		\centering
		\includegraphics[height=0.3\textheight]{trifase1}\\
		\caption{Circuiti equivalenti delle tre fasi e dei circuiti rotorici[1]}
	\end{figure}
	...
	2. La potenza elettrica, che è di nostro grande interesse, poiché esprime l' accoppiamento naturale che esiste tra la parte elettrica e la parte meccanica della macchina sincrona, infatti:\\
	...
	Dal punto di vista numerico le equazioni di sopra non sono molto maneggevoli. Infatti in caso di simulazione i valori delle grandezze sono molto elevati, creando appunto problemi al calcolatore. Inoltre sono soprattutto le grandezze di statore ad avere valori molto elevati, mentre quelle di rotore no; per rendere l' idea di questo fatto si consideri che la tensione ai terminali di statore può aggirarsi su valori intorno ai $10^5$-$10^6$ V, mentre quella ai capi del circuito di field potrebbe essere intorno ai $10^2$-$10^3$ V. Quindi solitamente si considerano dei valori di base delle grandezze attraverso i quali vengono normalizzati tutti gli altri, al fine di ottenere dei valori in per-unit (p.u.). Normalmente la scelta delle variabili di base viene fatta separatamente per le grandezze di statore e quelle di rotore, proprio per la differenza di ordini di grandezza che esiste tra di esse. 
	 ...
	 Come indicato in [1] sceglieremo come grandezze di base $S_B,V_B,\omega_B$, dove:\\
	 ...
	 $\frac{J\dot{\omega}}{\frac{S_{B3}}{\omega_B}}=\frac{C_m-C_e -C_d}{\frac{S_{B3}}{\omega_B}}\quad\rightarrow\quad2\frac{H}{\omega_B}\dot{\omega}=C_m -C_e -C_d\quad \text{(p.u)}$. \\Questa manipolazione è solitamente fatta perché $H$ è facilmente ottenibile attraverso una frazione di grandezze fisse conosciute a priori. Nell' equazione precedente non sono ancora normalizzate $\omega$ e il tempo, quindi occorrono ancora altri passaggi.\\Ricordando infatti che $t_u=\omega_Bt$ e che $\omega_u=\frac{\omega}{\omega_B}$, possiamo trasformare l' equazione precedente:\\
	 \begin{equation*}
	...
	\begin{figure}
		...
		\caption{Circuito transitorio equivalente[1]}
	\end{figure}
	...
	\end{cases}
	\]
	\chapter{La macchina in una rete di potenza}
	...
	\begin{figure}
		...
		\caption{Grafico della rete WSCC-9[1]}
	\end{figure}
	...
	\begin{figure}
		\centering
		\includegraphics[scale=0.3]{nonridotta}
	...
	\begin{figure}
		\centering
		\includegraphics[scale=0.33]{ridotta}
	...
	\begin{figure}
		\centering
		\includegraphics[scale=0.37]{cambio_coordinate1}
		\caption{Riferimento $V_{QD}$ e $V_{qd}$[1]}
	\end{figure}
	...
	\end{align*}
	Si tenga presente che d' ora in poi si parlerà di variazioni di grandezze, se non altrimenti specificato.
	\chapter{Controllo della macchina sincrona}
	\section{Generalità}
	...
	Per quanto riguarda la sede delle unità di controllo si possono distinguere tre diversi gruppi, come largamente spiegato in [3]:\\
	...
	$\frac{E_{fd}(s)}{e(s)}=k_p(1+\frac{1}{T_is}+\frac{T_ds}{1+\frac{T_d s}{N}})(\frac{k_a}{1+\tau_as})G_{ex}(s)=k_p(1+\frac{1}{T_is}+\frac{T_ds}{1+\frac{T_d s}{N}})(\frac{k_a}{1+\tau_as})(\frac{k_e}{1+\tau_es})$%
	}
	\end{equation*}
	Resta ora solo da capire come identificare la funzione di trasferimento del plant, cioè $\frac{E'_t(s)}{E_{fd}(s)}$. Guardando il modello con lo spazio di stato si hanno a disposizione come variabili di stato le tensioni in diretta e quadratura $E'_d$ e $E'_q$ delle varie macchine. L' obiettivo è sempre quello di lavorare nel dominio di Park, per non dover gestire funzioni sinusoidali del tempo, e quindi lavoreremo sul modulo del vettore $E'_t$, che è il vettore avente per componenti $E'_d$ e $E'_q$, pertanto il suo modulo è pari a:
	$|E'_t|=\sqrt{E'_{d}^2+E'_{q}^2}$. La relazione tra $E'_{t}$ e le tensioni in diretta e quadratura quindi non è lineare, ma visto che stiamo lavorando su un modello linearizzato, non bisogna dimenticarsi che stiamo parlando di variazioni di grandezze, quindi possiamo approssimare la relazione al primo ordine:\\
	\begin{equation*}
	E'_t=\frac{\partial E'_t}{\partial E'_q}E_q+\frac{\partial E'_t}{\partial E'_d}E'_d= \frac{2E'_{d0}}{2\sqrt{E'_{d0}^2+E'_{q0}^2}}E'_d+\frac{2E'_{q0}}{2\sqrt{E'_{d0}^2+E'_{q0}^2}}E'_q=\frac{E'_{d0}}{E'_{t0}}E'_d+\frac{E'_{q0}}{E'_{t0}}E'_q
	\end{equation*}
	...
	\end{equation}
	Con i dati in figura 2.8, sviluppando i calcoli e ricordando che per come è stato sviluppato il modello non siamo interessati a indagare sui controllori della macchina 1, si ottiene:\\
	\begin{equation*}
...
\newpage	
	...
	\end{figure}
 	...
 	\begin{equation*}
 	\frac{K}{1+T_1 s} e^{-\tau s}
 	\end{equation*}
 	dove $e^{-\tau s}$ è un fattore di ritardo.\\Per far ciò si utilizzerà un tool di stima della risposta a gradino presente nel toolbox di System Identification di Matlab, il cui risultato è riportato qui sotto (fig. 3.7).
 	\begin{figure}[H]
 		\centering
	...
	\end{figure}
	Attraverso l' uso della funzione "tuning\textunderscore migliore.m", (appendice A.1) è stato determinato il metodo più efficace, attraverso l' analisi della risposta della funzione ad anello chiuso $W=\frac{\text{PID}(s)\cdot P2(s)}{1+\text{PID}(s)\cdot P2(s)}$,di cui si riporta il risultato in figura 3.9.\\
	...
	\end{figure}
	Un altro punto da chiarire è la scelta del filtro in banda dell' azione derivativa N. Pertanto è stata scritta un' altra funzione, "seleziona\textunderscore N.m" (appendice A.2), che grafica le varie risposte del sistema al variare di N, e il risultato è riportato in figura 3.10.
\begin{figure}[H]
	\centering
	\includegraphics[scale=0.33,angle=90]{vari_N}
...
\end{figure}
	...
	\begin{equation*}
	PID_2=\frac{-20640s^2-19.47s-0.004793}{2493s^2+s}
	\end{equation*}
	Lavorando in per-unit è opportuno chiedersi se la funzione di trasferimento da origine a valori di guadagni e costanti di tempo che hanno effettivamente senso. Utilizzando la funzione "trova\textunderscore parametri.m" (appendice A.4), è possibile trovare i corrispettivi valori di $k_p$,$T_i$,$T_d$. In questo caso si ha:\\
	\[
	\begin{cases}
	k_p=-7.5248\\
	...
	\begin{figure}[H]
		\centering
	\includegraphics[scale=0.255]{rlocus_P2_pos}
	\includegraphics[scale=0.255]{rlocus_P2_neg}
	...
	Siamo ora interessati a vedere i margini di fase e di guadagno del sistema controllato. Si riporta in figura 3.14 il risultato ottenuto utilizzando Matlab.
	\begin{figure}[H]
		\centering
	...
	\end{equation*}
	Utilizzando le stesse regole di sintesi del PID, è stata riutilizzata di nuovo la funzione per trovare il tuning migliore (risultato in fig. 3.16).
	\begin{figure}[H]
		\centering
	...
	\begin{equation}
	PID_3=\frac{-10010s^2-11.1s-0.003209}{2125s^2+s}\rightarrow\begin{cases}
	k_p=-4.28\\
	...
	\caption{Modello completo in Simulink della rete con i blocchi dei controllori inseriti}
	\end{figure}
	...
	\caption{Dettaglio dello schema a blocchi relativo al sistema di controllo della macchina 2}
	\end{figure}
	\begin{figure}[H]
		\centering
	\includegraphics[scale=0.45,angle=-90]{dettaglio_controllore_3}
	\caption{Dettaglio dello schema a blocchi relativo al sistema di controllo della macchina 3}
	\end{figure}
	\newpage
	\section{Simulazioni}
	Di seguito (figura 3.24 e 3.25) sono riportate le simulazioni nel caso di inseguimento di un riferimento a gradino e nel caso di disturbi costanti.
	\begin{figure}[H]
		\centering
		\includegraphics[scale=0.255]{simulazione_riferimento}
		\caption{Inseguimento di un riferimento di 0.07 (macchina 2) e 0.05 (macchina 3)}
		\includegraphics[scale=0.255]{simulazione_disturbo}
		\caption{Inseguimento di un disturbo di 0.03 (macchina 2) e 0.05 (macchina 3)}
	\end{figure} \vfill\newpage
	Qui sotto vengono riportate in tabella le caratteristiche della risposta in caso di inseguimento.\\
	\begin{tabular}{|l|l|l|}
		\hline
	...
	\end{figure}
	 Per giustificare questi risultati ci sono da fare diverse considerazioni.\\In primo luogo, il controllo di questo sistema MIMO è stato effettuato utilizzando un approccio SISO, analizzando la funzione di trasferimento  $\frac{E'_{ti}(s)}{E_{fdi}(s)}\text{, con i=}2,3$, e quindi trascurando l' influenza che le altre variabili del sistema hanno sulla tensione d' uscita. Queste ultime inoltre sono lasciate incontrollate, e soprattutto le $\omega_i$ sono delle variabili che hanno una dinamica lenta come si può osservare in figura 3.27, e vanno ad influenzare la variabile controllata di interesse. Queste non lo fanno in maniera diretta, come si può constatare guardando lo spazio di stato generale del sistema interconnesso, ma intervengono nella dinamica dei vari $\delta_i$, che influenzano in maniera significativa le tensioni. Un altro aspetto da considerare è che il tuning dei controllori è stato eseguito in maniera obsoleta rispetto a quanto fanno gli algoritmi intelligenti che vengono impiegati oggi. I parametri del blocco di compensazione sono stati difatti determinati offline, in maniera definitiva una volta condotto lo studio, utilizzando delle regole empiriche. Sarebbe possibile, e anche più efficace, utilizzare delle tecniche di controllo ottimo per sistemi MIMO; un approccio di questo tipo difatti avrebbe il vantaggio di far cooperare tra loro i vari controllori. Il PID della seconda macchina, per esempio, applica la legge di controllo senza considerare l' azione che esercita il controllore della terza macchina. Questa situazione, proprio per la natura dell' AVR, aumenta l' instabilità delle $\omega_i$ e di conseguenza le oscillazioni dei $\delta_i$. Sempre in quest' ottica vengono molto utilizzate le reti neurali per la determinazione dei parametri dei controllori, e questi tipi di approcci determinano dei notevoli miglioramenti rispetto ai metodi tradizionali, come mostrato in [4].
	\appendix
	...
	\begin{figure}[H]
		...
		\caption{Individua il filtro migliore per l' azione derivativa}
	\end{figure}
	\begin{figure}[H]
	\includegraphics[scale=0.5]{tuning_selezionaN}
	\caption{Trova il tempo d' assestamento minore al variare del metodo di tuning e del filtro dell' azione derivativa}
	\end{figure}
	\begin{figure}[H]
		\includegraphics[scale=0.5]{trova_parametri}
		\caption{Trova il valore del guadagno proporzionale e delle costanti di tempo dell' azione integrale e di quella derivativa}
	\end{figure}
	\begin{figure}[H]
		\includegraphics[scale=0.5,angle=90]{migliora_integrale_script}
		\caption{Aumenta la costante di tempo dell' azione integrale nell' intento di diminuire la sovraelongazione non superando la soglia del tempo d' assestamento}
	\end{figure}
	...
	\begin{thebibliography}{9}
		...
		\bibitem{Neural networks for designing an automatic voltage regulator of a synchronous generator}
		Luyolo N. Magangane, Komla A. Folly,
		\textit{"Neural networks for designing an automatic voltage regulator of a synchronous generator"}. IEEE, 2013.
